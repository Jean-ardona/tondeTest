    ...
    <div className="relative inline-flex">
      ...
      >
        <div ref={dropdown} onFocus={() => setDropdownOpen(true)} onBlur={() => setDropdownOpen(false)}>
          ...
          <ul>
            {[
              { label: 'Documentation', icon: <><rect y="3" width="12" height="9" rx="1" /><path d="M2 0h8v2H2z" /></> },
              { label: 'Support Site', icon: <path d="M10.5 0h-9A1.5 1.5 0 000 1.5v9A1.5 1.5 0 001.5 12h9a1.5 1.5 0 001.5-1.5v-9A1.5 1.5 0 0010.5 0zM10 7L8.207 5.207l-3 3-1.414-1.414 3-3L5 2h5v5z" /> },
              { label: 'Contact us', icon: <path d="M11.854.146a.5.5 0 00-.525-.116l-11 4a.5.5 0 00-.015.934l4.8 1.921 1.921 4.8A.5.5 0 007.5 12h.008a.5.5 0 00.462-.329l4-11a.5.5 0 00-.116-.525z" /> },
            ].map(({ label, icon }) => (
              <li key={label}>
                <Link className="font-medium text-sm text-violet-500 hover:text-violet-600 dark:hover:text-violet-400 flex items-center py-1 px-3" to="#0" onClick={() => setDropdownOpen(!dropdownOpen)}>
                  <svg className="w-3 h-3 fill-current text-violet-500 shrink-0 mr-2" viewBox="0 0 12 12">{icon}</svg>
                  <span>{label}</span>
                </Link>
              </li>
            ))}
