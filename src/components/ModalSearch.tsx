...
function ModalSearch({ id, searchId, modalOpen, setModalOpen }: ModalSearchProps) {
  ...
  const recentPages = [
    { title: 'Messages', sub: 'Conversation / … / Mike Mills' },
    { title: 'Messages', sub: 'Conversation / … / Eva Patrick' },
  ];
            ...
            <div className="mb-3 last:mb-0">
              ...
              <ul className="text-sm">
                {recentSearches.map((item, i) => (
                  <li key={i}>
                    <Link className="flex items-center p-2 text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700/20 rounded-lg" to="#0" onClick={() => setModalOpen(!modalOpen)}>
                      {searchIcon}
                      <span>{item}</span>
                    </Link>
                  </li>
                ...
                {recentPages.map((page, i) => (
                  <li key={i}>
                    <Link className="flex items-center p-2 text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700/20 rounded-lg" to="#0" onClick={() => setModalOpen(!modalOpen)}>
                      ...
                      <span><span className="font-medium">{page.title}</span> - <span className="text-gray-600 dark:text-gray-400">{page.sub}</span></span>
                    </Link>
                  </li>
                ))}
              </ul>
