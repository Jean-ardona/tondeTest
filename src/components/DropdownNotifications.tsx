    ...
    <div className="relative inline-flex">
      ...
      >
        <div ref={dropdown} onFocus={() => setDropdownOpen(true)} onBlur={() => setDropdownOpen(false)}>
          ...
          <ul>
            {[
              { text: '📣 Edit your information in a swipe', body: 'Sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim.', date: 'Feb 12, 2024' },
              { text: '📣 Edit your information in a swipe', body: 'Sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim.', date: 'Feb 9, 2024' },
              { text: '🚀 Say goodbye to paper receipts!', body: 'Sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim.', date: 'Jan 24, 2024' },
            ].map((notif, i) => (
              <li key={i} className="border-b border-gray-200 dark:border-gray-700/60 last:border-0">
                <Link className="block py-2 px-4 hover:bg-gray-50 dark:hover:bg-gray-700/20" to="#0" onClick={() => setDropdownOpen(!dropdownOpen)}>
                  <span className="block text-sm mb-2">{notif.text} <span className="font-medium text-gray-800 dark:text-gray-100">{notif.body}</span></span>
                  <span className="block text-xs font-medium text-gray-400 dark:text-gray-500">{notif.date}</span>
                </Link>
              </li>
            ))}
          </ul>
